...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, service: 'pep-emr-api', port: PORT, database: 'postgresql' });
});
...
async function start() {
  ...
  app.listen(PORT, () => {
    ...
    if (!process.env.GOOGLE_CLIENT_ID) {
      console.warn('⚠ Google OAuth: copie .env.example → .env e preencha GOOGLE_CLIENT_ID/SECRET');
    } else {
      console.log(`Google OAuth callback: ${process.env.GOOGLE_REDIRECT_URI || 'não definido'}`);
    }
  });
}
